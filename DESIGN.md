...
| `8k` `1.5k` | **천원 단위** | `맥주 8k` = 8,000 |
| `8100` (3자리 이상 정수) | 원 단위 | `택시 8100` = 8,100 |
| `1100x2` `1100*2` `맥주 × 2` | 단가 x 수량 (기호가 떨어져 있어도 인식) | `아이시스500ML 1100x2` = 2,200 |
| `3.1x4` | 소수점 단가 x 수량 | `수원왕갈비 3.1x4` = 31,000 x 4 |
| `총 9.6` `합계` `총액` | **합계**. 품목이 아니다 | 품목 합보다 이 값을 신뢰한다 |
| 품목명 끝 숫자 (`소주2`) | 수량 (금액 아님) | `소주2 1.0` = 소주 2병 10,000 |
| 쉼표 | 항목 구분자 (공백과 동일) | `콘칩 2.5, 테라 5.5x3` |

**`총 N` 은 품목 합보다 우선한다.** 품목을 다 적지 않거나 할인이 섞인 경우가 있어 품목 합이
실제 결제액과 다르다. 이 값을 품목으로 잡으면 합계가 이중 계산된다.

> 🚨 **소수점의 단위는 문맥에 따라 다르고, 파서는 만원으로만 읽는다.** 수제맥주처럼 메뉴판
> 가격을 그대로 적는 경우(`골든 에일 4.9` = 4,900원)는 10배로 잡힌다. 같은 `4.9` 가 술자리
> 요리면 49,000원이라 문맥 없이 구분할 수 없다. 현재는 규칙을 정하지 않고 그런 건만 사후
> 보정한다 (실측: 임포트 67건 중 8건).
...
| 시각 | **콜론이 있는 것만** (`21:37`). 없으면 **비운다** | `2137` 은 금액과 구분 불가하므로 시각으로 보지 않는다 |
| 장소 | 첫 금액 이전의 선두 토큰 | `원조해장촌` |
| 품목 | `품목[수량] 금액` 패턴 반복 | `2인세트 4.5` |
| 사람 | `person` 테이블의 name/aliases 사전 매칭 | `박채원`, `정민` |
| 자유 서술 | 5개 이상 토큰이 뭉치면 품목이 아니라 **메모** | `한번에 적립된 듯한데 내역 확인이 안됨` |
...
**Keep 은 2단으로 적혀 있어 줄의 성격을 세 가지로 나눈다.** 복사해 붙여넣으면 들여쓰기가
사라져 상하관계를 알 수 없게 되므로, 날짜/시각 표기로 판단한다.

| 줄 | 처리 |
|---|---|
| 날짜만 (`6/12`, `6/12 (목)`) | **머리글**. 기록을 만들지 않고 아래 줄들이 물려받을 날짜로만 쓴다 |
| 시각만 (`21:41 택시 6700`) | 독립 기록이지만 **날짜를 위에서 물려받는다** |
| 날짜도 시각도 없음 | 직전 기록의 **하위 항목으로 합친다** |

합쳐진 기록의 원문은 줄바꿈으로 이어 붙여 원래 구조를 알아볼 수 있게 남긴다.

> 🚨 **임포트에서 시각을 지어내지 않는다.** 파서가 "시각이 없으면 입력 시각"으로 채우면
> 과거 메모를 넣을 때 실행 시각이 박힌다 (실측: 13건이 전부 `23:08` 로 저장됐다). 파서는
> 시각을 비워 돌려주고, 실시간 작성 경로에서만 현재 시각을 채운다.

실측 규모: 67줄을 넣었더니 13줄이 하위 항목인데 독립 기록이 됐고(한 자리의 주문 8건이 8개
기록), 4줄이 날짜를 물려받지 못해 임포트 당일로 저장됐다.
...
- **위 부류의 결함은 JVM 테스트로 잡히지 않는다.** JVM 에는 메타데이터가 그대로 있어 CI 가
  ...
  **바이너리를 실제로 띄워 로그인·저장·조회까지 왕복**한다 (7.6).
- 🚨 **`open-in-view=false` 이므로 서비스는 엔티티가 아니라 DTO 를 돌려준다.** 트랜잭션이
  끝나면 세션이 닫혀 컨트롤러에서 lazy 컬렉션을 건드리는 순간
  `LazyInitializationException` 이 된다. 변환을 트랜잭션 경계 안에서 끝내야 한다.
- **`@Transactional` 을 붙인 테스트는 이 경계를 검증하지 못한다.** 테스트 트랜잭션이 세션을
  열어둔 채 유지되어 lazy 로딩이 성공해 버린다(실측: 목록 조회가 전부 500 이었는데
  `EntryApiTests` 는 통과했다). 테스트는 격리를 위해 `@Transactional` 을 유지하고, 이
  경계는 스모크 테스트가 검증한다.
