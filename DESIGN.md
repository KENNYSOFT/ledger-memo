...
🚨 **날짜도 시각도 없는 줄은 직전 기록의 하위 항목으로 합친다.** Keep 에서 상하관계로 적어둔
주문 내역이 줄 단위로 쪼개지면 한 자리의 결제가 여러 기록이 되어 합계도 사람도 흩어진다
(실측: 67줄 중 13줄이 이렇게 흩어졌고, 한 자리의 주문 8건이 8개 기록이 됐다). 합쳐진 기록의
원문은 줄바꿈으로 이어 붙여 원래 구조를 알아볼 수 있게 남긴다.
